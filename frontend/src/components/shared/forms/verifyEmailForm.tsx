...
export default function VerifyEmailForm() {
    ...
    const handleResend = async () => {
        if (!email) return Swal.fire("Gagal", "Email tidak ditemukan.", "error");
    ...
    const handleVerify = async () => {
        const code = otp.join("");
        if (code.length !== 6) return Swal.fire("Error", "Masukkan kode OTP lengkap (6 digit).", "error");

        try {
            setLoading(true);
            const res: ApiResponse = await verifyEmail({ email: email!, otp: code });
            await Swal.fire("Berhasil", res.message, "success");
            router.push(isFromQuery ? "/login" : "/profile");
        } catch {
        ...
        <div className="flex flex-col items-center justify-center min-h-screen bg-gray-50 px-4">
            <div className="bg-white shadow-md rounded-2xl p-8 w-full max-w-md text-center">
                ...
                <div className="text-sm text-gray-500">
                    {resendTimer > 0 ? (
                        <p>Kirim ulang kode dalam <b>{resendTimer}</b> detik</p>
                    ) : (
