...
exports.createEvent = async (req, user) => {
    ...
    // ✅ Generate QR code unik dan expiry (misal 30 hari)
    const qrCode = uuidv4();
    const qrExpiresAt = dayjs().add(30, 'day').toDate();
    ...
    const qrText = `SIMKAS_EVENT_${qrCode}`;
    ...
    const uploadResult = await new Promise((resolve, reject) => {
        const uploadStream = cloudinary.uploader.upload_stream(
            {
                folder: "simkas/qrcodes",
                public_id: `event_${event.id}_qr`,
                resource_type: "image",
                overwrite: true,
            },
            (error, result) => {
